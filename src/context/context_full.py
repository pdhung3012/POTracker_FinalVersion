...
def format_chat(model_id, system_msg, context_str, user_prompt):
...
def generate_response(
    ...
    system_msg, context_str, user_prompt,
    max_attempts=3, max_input_tokens=None
):
    for attempt in range(max_attempts):
        prompt = format_chat(model_id, system_msg, context_str, user_prompt)
...
def main(model_id, input_path,context_path, output_path, max_attempts=3, max_input_tokens=None):
    ...
    input_data=json.load(open(input_path,'rb'))[:100]
    output_data = []

    index_item=-1
    for item in tqdm(input_data, desc="🧠 Generating responses"):
        index_item+=1
        system_prompt = InContextPromptTemplate.system_template
        context_prompt =str(json_context)
        # context_prompt = 'empty'
        # user_prompt = 'What is capital of Vietnam?'

        user_prompt=InContextPromptTemplate.prompt_template.replace('{input_XML}',item.get('input'))
